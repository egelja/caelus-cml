...
void meshOctreeCreator::setRootCubeSizeAndRefParameters()
{
    ...
    do
    {
        ...
        const scalar lSize = size / CML::pow(label(2), label(globalRefLevel_));

        if( lSize < (maxSize * (1.0-SMALL)) )
        {
            const scalar bbSize =
                0.5 * maxSize * CML::pow(label(2), label(globalRefLevel_));
    ...
    if( meshDictPtr_->found("boundaryCellSize") )
    {
        ...
        do
        {
            ...
            const scalar lSize = maxSize / CML::pow(label(2), addLevel);
            ...
            forAll(entries, dictI)
            {
                ...
                else if( patchDict.found("cellSize") )
                {
                    ...
                    do
                    {
                        ...
                        const scalar lSize = maxSize / CML::pow(label(2), nLevel);
