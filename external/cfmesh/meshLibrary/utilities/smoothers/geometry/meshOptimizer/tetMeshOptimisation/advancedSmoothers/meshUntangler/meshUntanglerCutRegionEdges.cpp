...
void meshUntangler::cutRegion::findNewEdges()
{
    ...
    forAll(edges, eI)
    {
        ...
        if( (newStartLabel != label(-1)) && (newEndLabel != label(-1)) )
        {
            newEdgeLabel_[eI] = cEdges.size();
            cEdges.append(edge(newStartLabel, newEndLabel));
        }
        else if( (newEndLabel != label(-1)) && (vertexTypes_[end] & KEEP) )
        {
        ...
        else if( (newStartLabel != label(-1)) && (vertexTypes_[start] & KEEP) )
        {
