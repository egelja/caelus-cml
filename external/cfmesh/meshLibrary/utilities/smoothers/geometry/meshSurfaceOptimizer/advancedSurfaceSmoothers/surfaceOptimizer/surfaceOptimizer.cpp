...
scalar surfaceOptimizer::evaluateStabilisationFactor() const
{
    ...
    forAll(trias_, triI)
    {
        ...
        Amin = CML::min(Amin, Atri);
        LsqMax = CML::max(LsqMax, LSqrTri);
    }
...
scalar surfaceOptimizer::evaluateFunc(const scalar& K) const
{
    ...
    forAll(trias_, triI)
    {
        ...
        const scalar Astab = CML::max(VSMALL, 0.5 * (Atri + stab));
        func += LSqrTri / Astab;
    }
...
) const
{
    ...
    forAll(trias_, triI)
    {
        ...
        const scalar Astab = CML::max(ROOTVSMALL, 0.5 * (Atri + stab));
...
scalar surfaceOptimizer::optimiseSteepestDescent(const scalar tol)
{
    ...
    const scalar avgEdge = CML::mag(pMax_ - pMin_);
    ...
    pMax_()
{
    ...
    forAll(trias_, triI)
    {
        ...
        for(label i=1;i<3;++i)
        {
            pMin_ = CML::min(pMin_, pts_[tf[i]]);
            pMax_ = CML::max(pMax_, pts_[tf[i]]);
        }
    }
}
