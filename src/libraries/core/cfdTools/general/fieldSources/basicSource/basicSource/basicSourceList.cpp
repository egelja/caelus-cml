...
void CML::basicSourceList::checkApplied() const
{
...
CML::basicSourceList::basicSourceList
(
    const fvMesh& mesh,
...
CML::basicSourceList::basicSourceList(const fvMesh& mesh)
:
    PtrList<basicSource>(),
    mesh_(mesh),
    checkTimeIndex_(mesh_.time().startTimeIndex() + 2)
...
void CML::basicSourceList::reset(const dictionary& dict)
{
    label count = 0;
...
bool CML::basicSourceList::read(const dictionary& dict)
{
...
bool CML::basicSourceList::writeData(Ostream& os) const
{
    // Write list contents
    forAll(*this, i)
    {
