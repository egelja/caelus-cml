...
CML::timeSelector::timeSelector()
:
    scalarRanges()
{}


CML::timeSelector::timeSelector(Istream& is)
:
    scalarRanges(is)
{}
...
bool CML::timeSelector::selected(const instant& value) const
{
    return scalarRanges::selected(value.value());
}


CML::List<bool> CML::timeSelector::selected(const instantList& Times) const
{
...
CML::List<CML::instant> CML::timeSelector::select(const instantList& Times)
const
{
...
void CML::timeSelector::inplaceSelect(instantList& Times) const
{
    inplaceSubset(selected(Times), Times);
}


void CML::timeSelector::addOptions
(
    const bool constant,
    const bool withZero
)
...
CML::List<CML::instant> CML::timeSelector::select
(
...
CML::List<CML::instant> CML::timeSelector::select0
(
    Time& runTime,
    const argList& args
)
...
CML::List<CML::instant> CML::timeSelector::selectIfPresent
(
    Time& runTime,
    const argList& args
)
