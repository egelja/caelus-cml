...
CML::dcLinearUpwindBJV<Type>::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "dcLinearUpwindBJV::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    forAll(faceFlux, facei)
    {
        ...
        scalar const limiter = max(0,min(min(1,4/(r+1+VSMALL)),4*r/(r+1+VSMALL)));
        ...
        if (pSfCorr.coupled())
        {
            ...
            forAll(pOwner, facei)
            {
                ...
                if (pFaceFlux[facei] > 0)
                {
                    ...
                    limiter = max(0,min(min(1,4/(r+1+VSMALL)),4*r/(r+1+VSMALL)));
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(dcLinearUpwindBJV, vector)
}
