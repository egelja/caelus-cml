...
#include "dcLinearUpwindMinmodV.hpp"
#include "fvMesh.hpp"
...
CML::dcLinearUpwindMinmodV<Type>::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "dcLinearUpwindMinmodV::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    forAll(faceFlux, facei)
    {
        ...
        scalar const limiter = max(min(r, 1), 0);
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(dcLinearUpwindMinmodV, vector)
}
