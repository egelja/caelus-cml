...
#include "dcLinearUpwindVanLeerV.hpp"
#include "fvMesh.hpp"
...
CML::dcLinearUpwindVanLeerV<Type>::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "dcLinearUpwindVanLeerV::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    forAll(faceFlux, facei)
    {
        ...
        scalar const limiter = 4*r/((r+1)*(r+1));
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(dcLinearUpwindVanLeerV, vector)
}
