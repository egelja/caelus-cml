...
public:
        ...
        )
    {
	  ...
	  beta_ = scalar(0.75);
        ...
        else
        {
            ...
            if (beta_ < 0 || beta_ > 1)
            {
                FatalIOErrorIn("blendedVanAlbada(fvMesh const&, Istream& is)", schemeData)
                    << "coefficient = " << beta_
                    << " should be >= 0 and <= 1"
                    << exit(FatalIOError);
            }
        ...
        )
    {
        if (schemeData.eof())
        {
            ...
            if (beta_ < 0 || beta_ > 1)
            {
	        FatalIOErrorIn("blendedVanAlbada(fvMesh&, surfaceScalarField const&, Istream& is)", schemeData)
                    << "coefficient = " << beta_
                    << " should be >= 0 and <= 1"
                    << exit(FatalIOError);
            }
