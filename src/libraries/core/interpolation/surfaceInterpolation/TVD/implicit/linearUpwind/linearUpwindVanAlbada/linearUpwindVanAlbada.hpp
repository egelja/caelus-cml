...
public:
        ...
        )
    {
        ...
        else
        {
            ...
            {
                FatalIOErrorIn("linearUpwindVanAlbada(fvMesh const&, Istream& is)", schemeData)
                        << "coefficient = " << order_
                        << " should be >= 0 and <= 1"
                        << exit(FatalIOError);
            }
        ...
        )
    {
        ...
        else
        {
            ...
            if (order_ < 0 || order_ > 1)
            {
                FatalIOErrorIn("linearUpwindVanAlbada(fvMesh const&, surfaceScalarField const& faceFlux, Istream& is)", schemeData)
                        << "coefficient = " << order_
                        << " should be >= 0 and <= 1"
                        << exit(FatalIOError);
            }
