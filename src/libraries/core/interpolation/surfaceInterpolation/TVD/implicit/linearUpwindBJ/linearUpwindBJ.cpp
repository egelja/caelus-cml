...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "linearUpwindBJ::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    forAll(faceFlux, facei)
    {
        ...
        scalar const limiter = max(min(min(1,4/(r+1+VSMALL)),4*r/(r+1+VSMALL)),0);
        ...
        if (pSfCorr.coupled())
        {
            ...
            forAll(pOwner, facei)
            {
                ...
                if (pFaceFlux[facei] > 0)
                {
                    ...
                    limiter = max(min(min(1,4/(r+1+VSMALL)),4*r/(r+1+VSMALL)),0);
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(linearUpwindBJ, scalar)
}
