...
CML::linearUpwindDLV<Type>::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "linearUpwindDLV::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    forAll(faceFlux, facei)
    {
        ...
        scalar const limiter = max(r*(r+2)/(r*(r+1)+2+SMALL), 0);
        ...
        if (pSfCorr.coupled())
        {
            ...
            forAll(pOwner, facei)
            {
                ...
                if (pFaceFlux[facei] > 0)
                {
                    ...
                    limiter = max(r*(r+2)/(r*(r+1)+2+SMALL), 0);
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(linearUpwindDLV, vector)
}
