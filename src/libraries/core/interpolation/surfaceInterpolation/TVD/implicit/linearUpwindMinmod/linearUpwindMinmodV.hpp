...
Class
    CML::linearUpwindMinmodV

Description
    A TVD property staisfying linear upwind scheme based on the Minmod 
    limiter to prevent unwanted oscillations. Implementation suitable for
    vector fields.

SourceFiles
    linearUpwindMinmodV.cpp
...
#ifndef linearUpwindMinmodV_H
#define linearUpwindMinmodV_H
    ...
    public upwind<Type>
{
    ...
    // Private Member Functions
        ...
        linearUpwindMinmodV(const linearUpwindMinmodV&);

        //- Disallow default bitwise assignment
        void operator=(const linearUpwindMinmodV&);


public:

    //- Runtime type information
    TypeName("linearUpwindMinmodV");
    ...
    // Constructors
            ...
            )
        {
            if (!schemeData.eof())
            {
                IOWarningIn
                (
                    "linearUpwindMinmodV(const fvMesh&, Istream&)",
                    schemeData
            ...
            )
        {
            if (!schemeData.eof())
            {
                IOWarningIn
                (
                    "linearUpwindMinmodV(const fvMesh&, "
