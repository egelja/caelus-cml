...
Description
    LinearUpwindMDBJV scheme class.

\*---------------------------------------------------------------------------*/

#include "mdLimiter.hpp"
#include "fvMesh.hpp"
#include "linearUpwindMDBJV.hpp"

template<class Type>
CML::tmp<CML::GeometricField<Type, CML::fvsPatchField, CML::surfaceMesh> >
CML::linearUpwindMDBJV<Type>::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "linearUpwindMDBJV::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    MDLimiter<BJLimiter,volVectorField> vectorLimiter
    (
        mesh,
        vf
    );
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(linearUpwindMDBJV, vector)
}
