...
Description
    LinearUpwindMDV scheme class.

\*---------------------------------------------------------------------------*/

#include "mdLimiter.hpp"
#include "fvMesh.hpp"
#include "linearUpwindMDV.hpp"

template<class Type>
CML::tmp<CML::GeometricField<Type, CML::fvsPatchField, CML::surfaceMesh> >
CML::linearUpwindMDV<Type>::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "linearUpwindMDV::correction(" + vf.name() + ')',
                mesh.time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    MDLimiter<VLimiter,volScalarField> scalarLimiter
    (
        mesh,
        vf
    );
...
namespace CML
{
    makelimitedSurfaceInterpolationTypeScheme(linearUpwindMDV, scalar)
}
