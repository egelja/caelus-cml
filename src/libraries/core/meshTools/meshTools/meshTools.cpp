...
CML::vectorField CML::meshTools::calcBoxPointNormals(const primitivePatch& pp)
{
    ...
    forAll(pointFaces, pointI)
    {
        ...
        else
        {
            WarningIn
            (
                "CML::meshTools::calcBoxPointNormals(const primitivePatch& pp)"
            ...
            else
            {
                ...
                WarningIn
                (
                    "CML::meshTools::calcBoxPointNormals"
...
)
{
    if (!hexMatcher().isA(mesh, cellI))
    {
        FatalErrorIn
        (
            "CML::meshTools::getCutDir(const label, const label)"
...
)
{
    if (!hexMatcher().isA(mesh, cellI))
    {
        FatalErrorIn
        (
            "CML::meshTools::getCutDir(const label, const vector&)"
        )   << "Not a hex : cell:" << cellI << abort(FatalError);
    }
