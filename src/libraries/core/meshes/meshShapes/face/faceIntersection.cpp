...
) const
{
    ...
    point ctr = CML::average(points(meshPoints));
    ...
    for (label pI = 0; pI < nPoints; pI++)
    {
        ...
        if (curHit.hit())
        {
            if (CML::mag(curHit.distance()) < CML::mag(nearestHitDist))
            {
...
CML::pointHit CML::face::intersection
(
    const point& p,
    const vector& q,
    const point& ctr,
...
CML::pointHit CML::face::nearestPoint
(
    const point& p,
    const pointField& meshPoints
) const
...
CML::pointHit CML::face::nearestPointClassify
(
    const point& p,
    const pointField& meshPoints,
    label& nearType,
...
) const
{
    ...
    for (label pI = 0; pI < nPoints; pI++)
    {
        ...
        if (CML::mag(curHit.distance()) < CML::mag(nearest.distance()))
        {
            nearest.setDistance(curHit.distance());
