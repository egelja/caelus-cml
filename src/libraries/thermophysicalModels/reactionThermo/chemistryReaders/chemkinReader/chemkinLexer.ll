...
int CML::chemkinReader::yyBufSize = YY_BUF_SIZE;
...
CML::string caelusSpecieString(const char* YYText)
{
    CML::string specieString(YYText);
...
CML::word caelusName(const char* YYText)
{
    CML::string fn(YYText);
    CML::string::stripInvalid<CML::word>(fn);
    return fn;
}


CML::word caelusName(const CML::string& s)
{
    CML::string fn(s);
    CML::string::stripInvalid<CML::word>(fn);
    return fn;
...
#define YY_DECL int CML::chemkinReader::lex()
...
<readElements>{elementName} {
        currentElementName = caelusName(YYText());
...
<readSpecies>{specieName} {
        word specieName(caelusName(caelusSpecieString(YYText())));
...
<readThermoSpecieName>{thermoSpecieName} {
        string specieString(caelusSpecieString(YYText()));
...
<readThermoFormula>{thermoFormula} {
        ...
        for (int i=0; i<4; i++)
        {
            word elementName(caelusName(thermoFormula(5*i, 2)));
...
<readThermoFormula2>{thermoFormula2} {
        string thermoFormula(YYText());
        word elementName(caelusName(thermoFormula(0, 2)));
...
<readReactionKeyword>{reactionKeyword} {
        ...
        word keyword(caelusName(YYText()));
        ...
        {
            currentSpecieName = caelusName(caelusSpecieString(YYText()));
...
<readPDependentSpecie>{pDependentSpecie} {
        ...
        pDependentSpecieName =
            caelusName(caelusSpecieString(YYText()));
        pDependentSpecieName =
...
<readReactionOrderSpecie>{specieName} {
        currentSpecieName =
            word(caelusName(caelusSpecieString(YYText())));
