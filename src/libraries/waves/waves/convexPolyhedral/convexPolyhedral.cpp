    ...
    forAll (eL, edgei)
    {
        ...
        if (pType[e.start()]*pType[e.end()] == -1)
        {
            ...
            else
            {
                ...
                newP = p0 + CML::mag(d0)/(CML::mag(d0) + CML::mag(d1))
                    *(p1 - p0);

                scalar dist(signedPointToSurfaceDistance(newP));
                label count(0);

                while (CML::mag(dist) > 5.0e-15)
                {
                    if (CML::sign(dist) == CML::sign(d0))
                    {
                        d0 = dist;
                        p0 = newP;
                    }
...
)
{
    ...
    else
    {
        ...
        if (cuttedEdges.size() >= 3)
        {
            ...
            if (CML::sign(f.normal( lc.points()) & normalToPlane_) == 1)
            {
...
{
    return CML::mag(s) < 5.0e-14 ? 0 : CML::sign(s);
}
...
)
{
    ...
    if (CML::sum(pType) == - CML::sum(pType*pType))
    {
        lc.fullNeg();
    }
    else if (CML::sum(pType) == CML::sum(pType*pType))
    {
        lc.fullPos();
    }
    else
