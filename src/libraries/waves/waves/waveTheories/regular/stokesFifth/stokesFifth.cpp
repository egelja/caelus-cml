...
) const
{
    ...
    scalar eta = (
                    eps*CML::cos(arg)
                  + pow(eps,2.0)*B22_*CML::cos(2.0*arg)
                  + pow(eps,3.0)*B31_*(CML::cos(arg) - CML::cos(3.0*arg))
                  + pow(eps,4.0)*
                    (
                        B42_*CML::cos(2.0*arg)
                      + B44_*CML::cos(4.0*arg)
                    )
                  + pow(eps,5.0)*
                    (
                        - (B53_ + B55_)*CML::cos(arg)
                        + B53_*CML::cos(3.0*arg)
                        + B55_*CML::cos(5.0*arg)
                    )
