...
if sys.platform == 'win32':
	logfile = open('buoyantBoussinesqSimpleSolver.log', 'w')
	run = subprocess.Popen(['mpiexec', '-localonly', '6', 'buoyantBoussinesqSimpleSolver', '-parallel'], stderr=logfile, stdout=logfile)
	run.wait()
	logfile.close()
	run = None
else:
	logfile = open('buoyantBoussinesqSimpleSolver.log', 'w')
	run = subprocess.Popen(['mpirun', '-np', '6', 'buoyantBoussinesqSimpleSolver', '-parallel'], stderr=logfile, stdout=logfile)
	run.wait()
	logfile.close()
	run = None
