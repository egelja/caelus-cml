...
run = subprocess.Popen(['potentialSolver', '-noFunctionObjects', '-writep'], stderr=logfile, stdout=logfile)
run.wait()
logfile.close()
run = None
...
if sys.platform == 'win32':
	...
	run = subprocess.Popen(['mpiexec', '-localonly', '6', 'simpleSolver', '-parallel'], stderr=logfile, stdout=logfile)
	run.wait()
	logfile.close()
	run = None
else:
	logfile = open('simpleSolver.log', 'w')
	run = subprocess.Popen(['mpirun', '-np', '6', 'simpleSolver', '-parallel'], stderr=logfile, stdout=logfile)
	run.wait()
	logfile.close()
	run = None
