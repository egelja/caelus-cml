#!/usr/bin/python
# ---------------------------------------------------------------------------
# Caelus 7.04
# Web:   www.caelus-cml.com
# ---------------------------------------------------------------------------

# Importing the required modules for Python
import subprocess
import sys
import os
import glob
import shutil
import Caelus

# Code name and version
code = 'Caelus'
version = Caelus.PROJECT_VER

# Starting up the meshing and solving
print "**********************************"
print "Starting %s %s simulation" % (code, version)
print "**********************************"

if sys.platform == 'win32':
   pltfrm = True
else:
   pltfrm = False

# Cleaning up the case
print "Cleaning case"
os.system('caelus-cleanCase.py')
os.system('caelus-clearPolyMesh.py')
try:
  shutil.rmtree("0")
except OSError:
  pass

# Copy of dynamicMeshDict
shutil.copy2('constant/dynamicMeshDict-SLERP', 'constant/dynamicMeshDict')

# Executing BlockMesh utility
print "Executing blockMesh"
run = subprocess.Popen(['caelus.py', '-l', 'blockMesh'], shell=pltfrm)
run.wait()
run = None

# Executing topoSet utility
print "Executing topoSet"
run = subprocess.Popen(['caelus.py', '-l', 'topoSet'], shell=pltfrm)
run.wait()
run = None

# Executing subsetMesh
print "Executing subsetMesh"
run = subprocess.Popen(['caelus.py', '-l', 'subsetMesh', '-overwrite', 'c0', '-patch', 'floatingObject'], shell=pltfrm)
run.wait()
run = None

# Copy of alpha1 field
shutil.copytree('0.org', '0')

# Copy of pointDisplacement field
shutil.copy2('0/pointDisplacement-SLERP', '0/pointDisplacement')


# Executing setFields
print "Executing setFields"
run = subprocess.Popen(['caelus.py', '-l', 'setFields'], shell=pltfrm)
run.wait()
run = None

# Executing vofDyMSolver
print "Executing vofDyMSolver"
run = subprocess.Popen(['caelus.py', '-l', 'vofDyMSolver'], shell=pltfrm)
run.wait()
run = None

# Create stub file for paraview
open('floatingObject.foam', 'a').close()
