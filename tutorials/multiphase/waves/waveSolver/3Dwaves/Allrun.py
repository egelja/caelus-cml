...
print "Executing setsToZones"
run = subprocess.Popen(['caelus.py', '-l', 'setsToZones','-noFlipMap'], shell=pltfrm)
run.wait()
run = None

print "Executing createBaffles"
run = subprocess.Popen(['caelus.py', '-l', 'createBaffles_old','f0','(internalWall internalWall)','-overwrite'], shell=pltfrm)
run.wait()
run = None
